...
def getFormulations(name, nyears):
    ...
    if nyears == 1000:
        ...
        formulation.reference = np.loadtxt('./../' + name + '/' + name +  \
                                           '_thinned_re-eval_1x100000.obj')
        formulation.reference = np.delete(formulation.reference,[2,4],1)
...
def makeFigures4_S2():
    nyears = [1000, 100000]
    figNames = ['Figure4.pdf', 'FigureS2.pdf']
    for i in range(len(nyears)):
        guidelines = getFormulations('guidelines_100',nyears[i])
        optimized = getFormulations('HydroInfo_100',nyears[i])
        ...
        ax.set_xlabel(r'$J_{Flood}$ (m above 11.25 m)', fontsize=16)
        ax.set_xlim([-0.1,3.3])
