    ...
    for perturbation in perturbations:
        for soln in solns:
            ...
            for day in range(days):
                dailyData = np.loadtxt('./perturbations/Soln' + str(soln) + '/r/Delta' + perturbation + \
                                       '/guidelines_100_day' + str(doy[day]) + '.txt')
                ...
                for i in range(len(perturbNames)):
                    posData = np.loadtxt('./perturbations/Soln' + str(soln) + '/r/Delta' + perturbation + \
                                         '/guidelines_100_pos_d' + perturbNames[i] + '_day' + str(doy[day]) + '.txt')
                    posData = normalizeInputs(posData[:,0:(nInputs+nOutputs)], IO_ranges)
                    if np.shape(posData)[0] > years:
                        posData = posData[0:years,:]
                    negData = np.loadtxt('./perturbations/Soln' + str(soln) + '/r/Delta' + perturbation + \
                                         '/guidelines_100_neg_d' + perturbNames[i] + '_day' + str(doy[day]) + '.txt')
            ...
            for output in range(nOutputs):
                ...
                np.savetxt('./sensitivities/Soln' + str(soln) + '/r/Delta' + perturbation + '/guidelines_100_' + \
